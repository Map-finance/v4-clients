...
describe('Validator Client', () => {
  ...
  describe('Transfers', () => {
    ...
    it('Withdraw', async () => {
      const tx = await client.post.withdraw(subaccount, 0, Long.fromInt(1_00_000_000), undefined);
      console.log('**Withdraw Tx**');
      console.log(tx);
    });

    it('Deposit', async () => {
      const tx = await client.post.deposit(subaccount, 0, Long.fromInt(1_000_000));
      console.log('**Deposit Tx**');
      console.log(tx);
    });

    it('Transfer', async () => {
      const tx = await client.post.transfer(subaccount, subaccount.address, 1, 0, Long.fromInt(1_000));
