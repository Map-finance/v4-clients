...
export class MsgClientImpl implements Msg {
  ...
  updateDowntimeParams(request: MsgUpdateDowntimeParams): Promise<MsgUpdateDowntimeParamsResponse> {
    ...
    const promise = this.rpc.request("h2x.blocktime.Msg", "UpdateDowntimeParams", data);
  ...
  updateSynchronyParams(request: MsgUpdateSynchronyParams): Promise<MsgUpdateSynchronyParamsResponse> {
    ...
    const promise = this.rpc.request("h2x.blocktime.Msg", "UpdateSynchronyParams", data);
    return promise.then(data => MsgUpdateSynchronyParamsResponse.decode(new BinaryReader(data)));
  }
}