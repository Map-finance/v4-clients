...
export class QueryClientImpl implements Query {
  ...
  marketMapperRevenueShareParams(request: QueryMarketMapperRevenueShareParams = {}): Promise<QueryMarketMapperRevenueShareParamsResponse> {
    ...
    const promise = this.rpc.request("h2x.revshare.Query", "MarketMapperRevenueShareParams", data);
    return promise.then(data => QueryMarketMapperRevenueShareParamsResponse.decode(new BinaryReader(data)));
  }
  marketMapperRevShareDetails(request: QueryMarketMapperRevShareDetails): Promise<QueryMarketMapperRevShareDetailsResponse> {
    const data = QueryMarketMapperRevShareDetails.encode(request).finish();
    const promise = this.rpc.request("h2x.revshare.Query", "MarketMapperRevShareDetails", data);
  ...
  unconditionalRevShareConfig(request: QueryUnconditionalRevShareConfig = {}): Promise<QueryUnconditionalRevShareConfigResponse> {
    ...
    const promise = this.rpc.request("h2x.revshare.Query", "UnconditionalRevShareConfig", data);
  ...
  orderRouterRevShare(request: QueryOrderRouterRevShare): Promise<QueryOrderRouterRevShareResponse> {
    ...
    const promise = this.rpc.request("h2x.revshare.Query", "OrderRouterRevShare", data);
