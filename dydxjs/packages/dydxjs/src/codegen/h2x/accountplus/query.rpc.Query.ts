...
export class QueryClientImpl implements Query {
  ...
  params(request: QueryParamsRequest = {}): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("h2x.accountplus.Query", "Params", data);
    return promise.then(data => QueryParamsResponse.decode(new BinaryReader(data)));
  }
  getAuthenticator(request: GetAuthenticatorRequest): Promise<GetAuthenticatorResponse> {
    const data = GetAuthenticatorRequest.encode(request).finish();
    const promise = this.rpc.request("h2x.accountplus.Query", "GetAuthenticator", data);
  ...
  getAuthenticators(request: GetAuthenticatorsRequest): Promise<GetAuthenticatorsResponse> {
    ...
    const promise = this.rpc.request("h2x.accountplus.Query", "GetAuthenticators", data);
  ...
  accountState(request: AccountStateRequest): Promise<AccountStateResponse> {
    ...
    const promise = this.rpc.request("h2x.accountplus.Query", "AccountState", data);
    return promise.then(data => AccountStateResponse.decode(new BinaryReader(data)));
  }
}
