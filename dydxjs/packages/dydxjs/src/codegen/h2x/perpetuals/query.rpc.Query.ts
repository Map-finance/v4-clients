...
export class QueryClientImpl implements Query {
  ...
  perpetual(request: QueryPerpetualRequest): Promise<QueryPerpetualResponse> {
    ...
    const promise = this.rpc.request("h2x.perpetuals.Query", "Perpetual", data);
  ...
  allPerpetuals(request: QueryAllPerpetualsRequest = {
    ...
    const promise = this.rpc.request("h2x.perpetuals.Query", "AllPerpetuals", data);
  ...
  allLiquidityTiers(request: QueryAllLiquidityTiersRequest = {
    ...
    const promise = this.rpc.request("h2x.perpetuals.Query", "AllLiquidityTiers", data);
  ...
  premiumVotes(request: QueryPremiumVotesRequest = {}): Promise<QueryPremiumVotesResponse> {
    ...
    const promise = this.rpc.request("h2x.perpetuals.Query", "PremiumVotes", data);
  ...
  premiumSamples(request: QueryPremiumSamplesRequest = {}): Promise<QueryPremiumSamplesResponse> {
    ...
    const promise = this.rpc.request("h2x.perpetuals.Query", "PremiumSamples", data);
    return promise.then(data => QueryPremiumSamplesResponse.decode(new BinaryReader(data)));
  }
  params(request: QueryParamsRequest = {}): Promise<QueryParamsResponse> {
    const data = QueryParamsRequest.encode(request).finish();
    const promise = this.rpc.request("h2x.perpetuals.Query", "Params", data);
    return promise.then(data => QueryParamsResponse.decode(new BinaryReader(data)));
  }
  nextPerpetualId(request: QueryNextPerpetualIdRequest = {}): Promise<QueryNextPerpetualIdResponse> {
    const data = QueryNextPerpetualIdRequest.encode(request).finish();
    const promise = this.rpc.request("h2x.perpetuals.Query", "NextPerpetualId", data);
